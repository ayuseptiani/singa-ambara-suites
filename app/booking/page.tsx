...
function BookingForm() {
  ...
  useEffect(() => {
    ...
    const fetchRoom = async () => {
      try {
        // Axios Call
        const res = await api.get(`/rooms/${roomSlug}`);
        const data = res.data;
        setRoom({ ...data, price: Number(data.price) });
      } catch (error) {
    ...
    if (checkIn && checkOut && room) {
      ...
      const checkRealtimeAvailability = async () => {
         ...
         try {
             ...
             const res = await api.get(`/check-availability?check_in=${checkIn}&check_out=${checkOut}&adults=1&children=0`);
             const data = res.data;
  ...
  }, [checkIn, checkOut, room, roomCount]);
  ...
  const handlePaymentAndBooking = async (e: React.FormEvent) => {
    ...
    try {
      // Axios Post (Header otomatis di-handle interceptor)
    ...
    <div className="w-full max-w-7xl mx-auto p-4 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
      <div className="lg:col-span-2 space-y-8">
        ...
        <form onSubmit={handlePaymentAndBooking} className="space-y-6">
          <div className={`p-6 rounded-xl border transition duration-300 ${!isAvailable ? 'bg-red-900/10 border-red-500' : 'bg-[#1A2225] border-gray-800'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="date" required min={getTodayString()} className="w-full bg-[#0F1619] border border-gray-700 rounded p-3 text-white focus:border-[#D4AF37] outline-none transition" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} />
              </div>
              <div>
                <label className="text-xs text-gray-400 uppercase tracking-widest">Check-Out</label>
                <input type="date" required min={checkIn || getTodayString()} className="w-full bg-[#0F1619] border border-gray-700 rounded p-3 text-white focus:border-[#D4AF37] outline-none transition" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} />
              </div>
              <div>
                <label className="text-xs text-gray-400 uppercase tracking-widest">Jml. Kamar</label>
                <select value={roomCount} onChange={(e) => setRoomCount(parseInt(e.target.value))} className="w-full bg-[#0F1619] border border-gray-700 rounded p-3 text-white focus:border-[#D4AF37] outline-none">
          ...
          <div className="bg-[#1A2225] p-6 rounded-xl border border-gray-800">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input type="text" required placeholder="Sesuai Kartu Identitas" className="w-full bg-[#0F1619] border border-gray-700 rounded p-3 text-white focus:border-[#D4AF37] outline-none placeholder-gray-600 transition" value={guestName} onChange={(e) => setGuestName(e.target.value)} />
              </div>
              <div className="space-y-2">
                <label className="text-xs text-gray-400 uppercase tracking-widest">Nomor WhatsApp</label>
                <input type="tel" required placeholder="Contoh: 081234567890" className="w-full bg-[#0F1619] border border-gray-700 rounded p-3 text-white focus:border-[#D4AF37] outline-none placeholder-gray-600 transition" value={phone} onChange={(e) => setPhone(e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#1A2225] p-6 rounded-xl border border-gray-800">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {['QRIS', 'BCA', 'Mandiri', 'CC'].map((method) => (
                <div key={method} onClick={() => setPaymentMethod(method)} className={`p-4 rounded border cursor-pointer flex flex-col items-center justify-center text-sm font-bold transition h-20 text-center ${paymentMethod === method ? 'bg-[#D4AF37]/20 border-[#D4AF37] text-[#D4AF37]' : 'bg-[#0F1619] border-gray-700 text-gray-400 hover:border-gray-500'}`}>
                  {method}
                </div>
              ))}
            </div>
          ...
          <button type="submit" disabled={isLoading || nights <= 0 || isChecking || !isAvailable} className={`w-full py-5 rounded-lg font-bold text-lg uppercase tracking-widest transition duration-300 shadow-xl ${isLoading || !isAvailable || isChecking ? 'bg-gray-600 text-gray-400 cursor-not-allowed opacity-50' : 'bg-[#9F8034] hover:bg-[#8A6E2A] text-white'}`}>
            {isLoading ? "Memproses..." : isChecking ? "Cek Stok..." : !isAvailable ? "Stok Tidak Cukup" : `Bayar Total: ${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits:0 }).format(totalPrice)}`}
          </button>
        </form>
      </div>
      ...
      <div className="lg:col-span-1">
         <div className="bg-[#1A2225] p-6 rounded-xl border border-[#D4AF37]/20 sticky top-24 shadow-2xl">
            ...
            <div className="space-y-3 text-sm text-gray-300 border-t border-gray-700 pt-4 mt-4">
               <div className="flex justify-between"><span>Harga/malam</span><span>{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits:0 }).format(room.price)}</span></div>
               <div className="flex justify-between"><span>Durasi</span><span className="text-white">{nights} Malam</span></div>
               <div className="flex justify-between"><span>Jumlah Kamar</span><span className="font-bold text-[#D4AF37]">x {roomCount} Unit</span></div>
               <div className="flex justify-between text-[#D4AF37] text-xl font-bold pt-4 border-t border-gray-700 mt-2"><span>Total Bayar</span><span>{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits:0 }).format(totalPrice)}</span></div>
            </div>
         </div>
      </div>
    </div>
